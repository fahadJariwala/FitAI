...
const ProgressScreen = () => {
    ...
    const chartData = {
        labels: progress.slice(-7).map(p => new Date(p.date).toLocaleDateString('en-US', { day: '2-digit' })),
        datasets: [{
            ...
            {progress.length > 0 ? (
                <>
                    ...
                    <Text style={styles.stats}>
                        Total Time: {progress.slice(-7).reduce((sum, p) => sum + p.minutes_spent, 0)} minutes
                    </Text>
                </>
            ) : (
